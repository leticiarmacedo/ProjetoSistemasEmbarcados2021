...
double compute_EAR1(std::vector<cv::Point> vec) // Razão de aspecto do olho direito 
{
...
double compute_EAR2(std::vector<cv::Point> vec) // Razão de aspecto do olho esquerdo
{
	...
	// Define o Detector de face e monta o modelo de aspectos do rosto.
        ...
        while (!win.is_closed()) {
            ...
            if (faces.size() > 0) {
                ...
                //threads para calcular a razão de aspecto dos olhos 
